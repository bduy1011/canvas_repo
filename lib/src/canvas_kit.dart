...
class CanvasKit extends StatefulWidget {
  ...
  /// When true, [CanvasKit] does not call [setState] on [CanvasKitController]
  /// notifications. The parent must listen to the same controller and call
  /// [setState] so pan/zoom and drag blocking stay in sync. Removes duplicate
  /// rebuilds when the parent already rebuilds on every controller tick.
  final bool externalTransformNotifications;
  ...
  const CanvasKit({
    ...
    this.onRenderStats,
    this.externalTransformNotifications = false,
  });
...
class CanvasKitController extends ChangeNotifier {
  ...
  void setScale(double nextScale, {Offset focalWorld = Offset.zero}) {
    ...
    if (bounds != null && _viewportSize != null) {
      minScaleAllowed = math.max(
        minZoom,
        getMinimumScaleForBounds(_viewportSize!),
      );
    }
  ...
  void fitToWorldRect(
    ...
    if (bounds != null) {
      minScaleAllowed = math.max(
        minZoom,
        getMinimumScaleForBounds(viewportSize),
      );
    }

    final targetScale = (sx < sy ? sx : sy)
        .clamp(minScaleAllowed, maxZoom)
        .toDouble();
...
class _CanvasKitState extends State<CanvasKit> {
    ...
    if (!_isInitialized && widget.autoFitToBounds && widget.bounds != null) {
      ...
      WidgetsBinding.instance.addPostFrameCallback((_) {
        ...
        if (renderBox != null && renderBox.hasSize) {
          _controller!.fitToBounds(
            renderBox.size,
            padding: widget.boundsFitPadding,
          );
        }
      });
    }
            ...
            children: [
              ...
              ...widget.foregroundLayers.map(
                (painterBuilder) => Positioned.fill(
                  child: IgnorePointer(
                    child: CustomPaint(painter: painterBuilder(transform)),
                  ),
                ),
              ),
                ...
                Positioned.fill(
                  child: GestureDetector(
                    ...
                    onScaleStart: (details) {
                      ...
                      _focalWorldAtStart = _controller!.screenToWorld(
                    ...
                    onScaleUpdate: (details) {
                      ...
                      if (widget.interactionMode ==
                        ...
                        _controller!.setScale(
                          proposed,
                          focalWorld: _focalWorldAtStart!,
                        );

                        final currentScreen = _controller!.worldToScreen(
                          _focalWorldAtStart!,
                        );
                        final screenDelta =
  ...
  void _onControllerChanged() {
    ...
    if (widget.externalTransformNotifications) {
      if (rev != _lastTransformRevision) {
...
class SimpleCanvas extends StatelessWidget {
  ...
  Widget _buildWith({
    ...
    Rect visibleWorldRectForTransform(Matrix4 t) {
      ...
      final br = Vector3(viewportSize.width, viewportSize.height, 0)
        ..applyMatrix4(inv);
      return Rect.fromLTRB(
        math.min(tl.x, br.x),
        math.min(tl.y, br.y),
    ...
    final worldVisible =
        (controller?.getVisibleWorldRect(viewportSize) ??
            ...
            .inflate(250.0);
    final screenVisible = Rect.fromLTWH(
      0,
      0,
      viewportSize.width,
      viewportSize.height,
    ...
    return Stack(
      ...
      children: [
        Stack(clipBehavior: Clip.none, children: worldItems),
        Stack(clipBehavior: Clip.none, children: viewportItems),
      ],
    );
  }
}
