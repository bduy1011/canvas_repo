...
class AngleCheck {
  public static boolean areNativeLibrariesLoaded = false;
  ...
  public static boolean isAllowed() {
    if (!areNativeLibrariesLoaded || !isVersionAllowed() || !isArm64() || isBlacklistedForAngle()) {
      return false;
    } 
    return true;
  }
...
public class FlutterAnglePlugin implements FlutterPlugin, MethodCallHandler {
  ...
  static {
    try {
      ...
      AngleCheck.areNativeLibrariesLoaded = true;
      Log.i(TAG, "Native ANGLE libraries loaded successfully");
    } catch (UnsatisfiedLinkError e) {
      AngleCheck.areNativeLibrariesLoaded = false;
  ...
  public void onDetachedFromEngine(@NonNull FlutterPlugin.FlutterPluginBinding binding) {
    ...
    try {
      if (AngleCheck.areNativeLibrariesLoaded) {
